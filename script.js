...
function setResult(result, currentButton) {
  ...
  // Accept any QR code for tasks 2 and 4
...
async function openCamera(currentButton) {
  ...
  if (currentButton === 0 || currentButton === 1) {
    ...
    stream = await navigator.mediaDevices.getUserMedia({
      audio: false,
      video: { minAspectRatio: 1.333, facingMode: "user" },
    });
    videoElement.srcObject = stream;
  } else if (currentButton === 2 || currentButton === 4) {
    // Create fresh video element for QR scanning
    ...
    // Real QR scanning - accepts any QR code
    ...
    scannerQr.start().then(() => {
      ...
      navigator.mediaDevices.getUserMedia({ video: true }).then((stream) => {
        ...
        setInterval(() => {
          QrScanner.scanImage(qrVideo, { returnDetailedScanResult: true })
            .then((result) => {
              console.log("QR Found:", result.data);
